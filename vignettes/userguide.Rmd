...
If your container is based on the same OS as the HPC, such as CentOS 7, you may
be able to bind in the scheduler executable to the container.
...
If the container image is based on a different OS, then binding in the
executable may still fail.  A more general solution is to create a script that
uses SSH to execute the scheduler on the login node.
...
2. Create a set of passphrase-less SSH keys that only exist on the HPC.
  A note on security: These SSH keys are not protected by a passphrase.  Anyone
  who has the private key file can log in as you on the HPC, and there is no
  passphrase protecting the private key.  Don't create these keys for a login
  that has admin privileges.  We will reduce the risks by only ever storing the
  keys in your home directory on the HPC and by restricting the
  `~/.ssh/authorized_keys` file to only allow logins from within the HPC
  clusters.  Restricting the keys to only allow ssh logins from within the HPC
  cluster, and only having a regular user account, means anyone using these
  keys must already be logged in to the HPC cluster, and probably won't gain
  much by logging in again as you.  Anyone who can access your ~/.ssh/* files
  on the cluster probably already has more privileges to the HPC than your
  account offers.
...
```
  ...
  Open `~/.ssh/authorized_keys` in a text editor.  Find the line corresponding
  to the local key you just added.  It will have username@node.on.your.hpc.edu
  at the end, or a similar name depending on the hostname of the node you ran
  the `ssh-copy-id` from.  Add `from="*.local"` to the beginning of the line to
  restrict access to logins originating from within the HPC.  Study
  `/etc/hosts` to find a good pattern, `"*.local"` may not work for you.

  You should now have a set of keys that allow automated ssh connections from
  the container to the login node.
3. Create scripts that call the scheduler on the login node via ssh.
  Such as `~/bin/qsub`. An example for the PBS `qsub`:
