...
For any installation issues please see the [Troubleshooting](#trouble-install) section.
...
If this does not work for any reason, please see the
...
### Installation errors {#trouble-install}
...
most Linux distributions). You can check your version using:

```{sh eval=FALSE}
# the minimum required gcc version is 4.8.1 for full C++11 support (clang 3.3)
cc --version
```
...
### Job submission fails with template error {#trouble-template}
...
specify which scheduler and template you want to use.

Be sure to know what scheduler your HPC provides, and then see the [manual
scheduler setup](#scheduler-templates) on how to set it up.
...
filled template from the error message (as this is an HPC scheduler template
issue, not a `clustermq` issue).

### Session gets stuck at "Running calculations" {#trouble-stuck}
...
This likely means that your jobs are started, but can not connect back to the
main session. You can confirm this by setting
[`log_worker=TRUE`](#debugging-workers) and inspect the logs created in your
current working directory. They will state something like:
...
Here, the submitted job is unable to establish a network connection with the
head node. This can happen if your HPC does not allow these connections, but
more likely happens because there are multiple network interfaces, only some of
which have access to the head node.

You can list the available network interfaces using the `ifconfig` command.
Find the interface that shares a subnetwork with the head node and add the
[option](#options) `clustermq.host=<interface>`. Contact your system
